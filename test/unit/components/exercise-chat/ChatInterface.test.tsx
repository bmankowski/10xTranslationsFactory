...
describe("ChatInterface User Interactions", () => {
  ...
  it("submits answer when user enters text and clicks send", async () => {
    ...
    render(<ChatInterface {...defaultProps} onAnswerSubmit={onAnswerSubmit} />);
    ...
    // Assert
    expect(onAnswerSubmit).toHaveBeenCalledWith("Hello, my name is John", mockQuestion.id);
  });
  ...
  it("disables submit button during loading state", () => {
    // Arrange
    render(<ChatInterface {...defaultProps} isLoadingSubmission={true} />);
