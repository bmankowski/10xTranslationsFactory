...
export class OpenRouterService {
  ...
  constructor(config?: Partial<OpenRouterConfig>) {
    ...
    this._config = {
      ...
      defaultModel: config?.defaultModel || '4o-mini'
    };

    // Initialize response format (not part of config)
    this.responseFormat = {
      type: 'json_schema',
      json_schema: {
        name: 'chat_response',
    ...
    if (config?.apiKey && config?.apiEndpoint) {
      this.initService(config as OpenRouterConfig);
    }
  }
  ...
  public get defaultModel(): string { return this._config.defaultModel || '4o-mini'; }
   ...
   */
  public static createInstance(config: OpenRouterConfig): OpenRouterService {
   ...
   */
  public async sendMessage(userMessage: string, additionalParams?: ModelParams): Promise<any> {
    // Validate input
   ...
   */
  public parseResponse(response: any): ParsedResponse {
    ...
    try {
      ...
      if (typeof content === 'string' && (content.trim().startsWith('{') || content.trim().startsWith('['))) {
        try {
          ...
          if (parsedContent.text && parsedContent.language) {
            return {
          ...
          // Not valid JSON, will fall back to treating as plain text
        }
      }
      
      // Handle plain text response - assume it's English
      if (typeof content === 'string') {
        ...
        // If content is already an object with our expected fields
      ...
      throw new Error('Unable to extract text content from OpenRouter response');
    } catch (error) {
      this.handleError(error);
   ...
   */
  private _buildRequestPayload(userMessage: string, additionalParams?: ModelParams): RequestPayload {
   ...
   */
  private async _performApiCall(payload: RequestPayload): Promise<any> {
    let retryCount = 0;
      ...
      try {
        ...
        const response = await fetch(this.apiEndpoint, {
          ...
          headers: {
            ...
            'X-Title': '10x Translations Factory'
          },
