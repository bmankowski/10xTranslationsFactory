...
import { TextResponseSchema } from "../lib/services/openRouterTypes";
  ...
  try {
    ...
    if (responseData.text) {
      return (
        <div>
          ...
          <p className="mt-2 text-sm text-gray-600">Language: {responseData.language_of_response || "Not specified"}</p>
          {responseData.arithmetical_value !== undefined && responseData.arithmetical_value !== null && (
            <p className="mt-2 text-sm text-gray-600">Value: {responseData.arithmetical_value}</p>
          )}
        </div>
      );
    }
    ...
    return (
      <div>
        ...
        <pre className="bg-gray-100 p-2 text-xs overflow-auto">{JSON.stringify(responseData, null, 2)}</pre>
      </div>
    );
  } catch (err) {
    return (
...
export default function OpenRouterTestChat() {
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto bg-white rounded-lg shadow-md">
      ...
      {error && <div className="mt-4 p-3 bg-red-100 text-red-800 rounded">Error: {error}</div>}
      ...
      {showDebug && rawResponse && (
        <div className="mt-4">
          ...
          <pre className="mt-2 p-3 bg-gray-800 text-white rounded text-xs overflow-auto">{rawResponse}</pre>
        </div>
      )}
    </div>
  );
